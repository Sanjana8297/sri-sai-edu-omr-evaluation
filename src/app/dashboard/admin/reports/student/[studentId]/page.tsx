...
function NotesTab({ studentId }: { studentId: string }) {
  const { data, isLoading } = useQuery({
    queryKey: ["admin", "student-sessions", studentId],
    queryFn: async (): Promise<SessionsResponse> => {
      const res = await fetch(`/api/admin/students/${encodeURIComponent(studentId)}/sessions`);
...
export default function StudentReportPage() {
  ...
  return (
    <>
      <div className="mb-4">
        <Link
          href="/dashboard/admin/reports?section=results"
          onClick={(e) => {
